...
RegisterCommand("sell", function(source, args, rawCommand)
    ...
    local character = NDCore.Functions.GetPlayerData(playerId, "char")
    local characterId = character.id
    local ownedProperty = nil
    -- Find the property owned by the player
    for _, property in pairs(properties) do
        if property.owner == characterId then
            ownedProperty = property
            break
        end
    end
    if not ownedProperty then
        TriggerClientEvent("chatMessage", playerId, "^1Error: You don't own any properties.")
        return
    end
    if args[1] == "house" then
        -- Calculate the sell price based on the original purchase price
        local sellPrice = ownedProperty.price
        -- Update property ownership and sale status
        ownedProperty.owner = nil
        ownedProperty.sale = 0
        -- Save the updated property information in the database
        MySQL.Async.execute("UPDATE nd_properties SET `owner` = NULL, `sale` = 0 WHERE `id` = @propertyId",
            {["@propertyId"] = ownedProperty.propertyid},
            function(rowsChanged)
                if rowsChanged > 0 then
                    TriggerClientEvent("chatMessage", playerId, "^2Success: You have sold your house for $" .. sellPrice)
                    TriggerClientEvent("ND_Properties:updateUI", -1, ownedProperty.propertyid, nil)
                else
                    TriggerClientEvent("chatMessage", playerId, "^1Error: Failed to sell the house.")
                end
            end
        )
    end
